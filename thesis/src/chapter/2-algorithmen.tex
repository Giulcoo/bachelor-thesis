...
Die verschiedenen Parameter und Größen der Daten wurden mit JMH getestet. JMH ist ein Java-Harness zum Erstellen von Benchmarks. In diesen Benchmarks können die Laufzeiten von Funktionen und Programmen gemessen werden, indem diese über mehrere Durchläufe ausgeführt werden und am Ende eine durchschnittliche Laufzeit zurückgegeben wird. Diese wird bei JMH in "ops/s" (Operationen pro Sekunde), also wie oft die zu testende Funktion pro Sekunde ausgeführt werden kann.\cite{githubGitHubOpenjdkjmh}

Bei den Ergebnissen\footnote{Alle Ergebnisse der Messungen der verschiedenen Strategien: \url{https://github.com/Giulcoo/bachelor-thesis/blob/main/resources/Strategies\%20Results/Score.txt}} in der Abbildung \ref{fig:topStrat} sind die zehn besten Strategien aufgezählt. Der Score-Wert wurde berechnet, indem \dots
\todo{Continue}
