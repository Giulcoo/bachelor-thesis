...
Warum so wenig hier erwähnen (Speichern deutlich komplexer und manche Speicherstrategien beeinflussen Laden(???)) 
\todo{Einleitung}
%--------------------------------------------------------------------------
...
Bevor die Daten geladen werden können sollten diese überprüft werden. Wenn dies nicht vor dem Laden der Daten gemacht wird, kann es zu fehlerhaften Zuständen im Spiel oder allgemein zu Problemen der Ausführung des Programmes kommen. Dabei gibt es zwei Kontrollen, die durchgeführt werden sollten.

Eine \textit{Versionskontrolle} überprüft die Version der Daten eines Spielstandes. Wenn diese Version älter als die Version der Anwendung ist, sollten die Daten zu der neuen Version migriert werden. Dafür ist es sinnvoll beim Speichern der Daten eine Version des Spiels mitzugeben, welche dann beim Laden vergleicht werden kann. Wenn die gespeicherte Version nicht mit der Version des Spieles übereinstimmen, müssen die alten Daten auf die neuen Datenstrukturen übersetzt werden. Dies kann ein sehr aufwendiger Prozess sein, weshalb die Datenstrukturen gut durchdacht sein und nicht häufig verändert werden sollten. 

Eine \textit{Korruptionskontrolle} überprüft, ob die Daten in irgendeiner Form beschädigt sind. Dies kann durch Fehler der Anwendung selber passieren, oder wenn Benutzer die Daten bearbeitet haben.
%--------------------------------------------------------------------------
...
%--------------------------------------------------------------------------
\section{Strategien}
Aus den Strategien der vorherigen sections verschiedene Speicher- und Ladesysteme aufstellen und deren Effizienz auswerten. \todo{Finish}
...
\footnote{Code zu den Strategien und Benchmarks: \url{https://github.com/Giulcoo/bachelor-thesis/tree/main/projects}}
\todo{Einleitung}
...
Was bei der Abbildung \ref{fig:topStrat} direkt auffällt, ist, dass die besten Strategien alle eine binäre Serialisierung verwenden. Dies liegt daran, dass die zu schreibenden und lesenden Daten mit der binären Serialisierung deutlich kompakter sind, als mit JSON.  

Wenn die besten Strategien von der binären- und JSON-Serialisierung betrachtet werden (siehe Abbildungen \ref{fig:topStratBin} und \ref{fig:topStratJson}), fällt außerdem noch auf, dass wenige Strategien mit \ac{gzip}-Komprimierung dabei sind. \ac{gzip} reduziert zwar die Anzahl der Schreib- und Leseprozesse im Sekundärspeicher, jedoch kann der Komprimierungsalgorithmus die Laufzeit so erhöhen, dass es schneller ist, direkt unkomprimiert mit den Daten im Sekundärspeicher zu arbeiten. Selbst bei einer großen Datenmenge (siehe Abbildung \ref{fig:bigDataCount}) scheinen Strategien mit \ac{gzip}-Komprimierung nicht effizienter als die ohne zu sein. Das einzige Szenario, bei der die \ac{gzip}-Komprimierung vorteilhafter ist, ist bei dem Szenario "removePlayers" (siehe Abbildung \ref{fig:removePlayers}). Hier verwenden die besten Strategien größtenteils die Komprimierung. Allgemein ist dieses Szenario das zeitintensivste, daher bietet die Reduzierung von Prozessen im Sekundärspeicher durch die Komprimierung einen Vorteil im Vergleich zu anderen Ansätzen. 

Außerdem scheinen Strategien mit dynamischer Chunk-Anzahl im allgemeinen besser abzuschneiden, die diese anpassungsfähiger sind und somit die Daten effizienter gruppieren. Die Strategie mit statischer Chunk-Anzahl, bei der die Spielwelt in zwei mal zwei, also vier Chunks, aufgeteilt wird, ist unter den besten zehn Strategien die einzige mit statischer Chunk-Anzahl. Es ist also effizienter, bei einer statischen Chunk-Anzahl, mit weniger Chunks zu arbeiten, statt die Spielwelt in vielen kleinen Chunks aufzuteilen. Dies fällt auch bei den Ergebnissen der Strategien mit dynamischer Chunk-Anzahl auf. Je mehr Player-Objekte ein Chunk beinhalten kann, desto effizienter läuft der Algorithmus.

Ein weiteres Ergebnis, welches unerwartet ist, ist, dass bei den besten Strategien keine dabei ist, die eine Change-Datei verwendet. Durch die minimierte Größe an Daten, die geschrieben werden müssen, ist eigentlich zu erwarten, dass dieser Ansatz besonders effizient läuft. Jedoch gibt es durch diesen Ansatz mehr Schreibprozesse. Und wenn die Daten der Schreibprozesse aufsummiert werden, ist die Größe dieser etwas größer als bei den Strategien ohne Change-Datei. Jedes Objekt der Change-Klasse bringt nämlich noch weitere Attribute mit, die gespeichert werden müssen.

\subsubsection{Interpretation}
Warum was\todo{Finish}

\subsubsection{Fazit}
\begin{itemize}
    \item Für ein anpassungsfähiges System dynamische Chunk-Anzahl
    \begin{itemize}
        \item Jedoch aufwendiger zu programmieren!
        \item Alternative Statisch mit wenig Chunks (z.B. 2x2)
    \end{itemize}
    \item Wenn beste Effizienz erreicht werden soll -> Binary + Dynamic
    \item Wenn Effizienz, aber lesbare Daten -> Json + Dynamic
    \item Wenn Datengröße gering gehalten werden soll -> Binary + Gzip + Dynamic
\end{itemize}
\todo{Finish}