...
Eine Möglichkeit, Daten zu komprimieren, ist mit dem populären patentfreien Kompressionsprogramm \ac{gzip}.\todo{Quelle} \ac{gzip} verwendet den Deflate-Algorithmus, der eine Kombination aus dem \ac{lz77}-Algorithmus und der Huffman-Kodierung ist.\todo{Fußnoten} Die komprimierten Daten befinden sich in einer \ac{gzip}-Datei, die die Daten als 32 Kilobytes Blöcke analysiert. Sobald sich in einem Block Wörter wiederholen, kann dieser Block komprimiert werden. \cite{gnuGzip}\cite{1414952}\cite{seobilityGzipFunktioniert}
...
Die verschiedenen Parameter und Größen der Daten wurden mit JMH getestet.\cite{githubGitHubOpenjdkjmh}\todo{Abkürzung} 
