...
public class DynamicChunkService extends ChunkService {
    ...
    protected void checkChunk(Chunk chunk){
        if(chunk == null) return;
        Chunk parentChunk = chunks.get(chunk.getParentChunk());
    ...
    protected void splitChunk(Chunk parentChunk){
        ...
        parentChunk.getPlayersList().forEach(p -> addPlayerToChunk(p, findChunk(p.getPosition(), childChunks), parentChunk.getId()));
    ...
    protected Chunk findChunk(Vector position, String currentChunkID){
        ...
        Chunk currentChunk = getChunk(currentChunkID);
